...
let TGCToastDefaultTime: Double = 1
...
extension UIView {
     ...
     */
    func tgc_makeToast(message: String, duration: Double = TGCToastDefaultTime , position:ToastPosition = ToastPosition.bottom) {
    ...
    @objc func tgc_onToHideToastView(timer: Timer) {
        self.tgc_hideWrapperView(view: timer.userInfo as! UIView, force: false)
    }
    
    @objc func tgc_onToHideErrorToastView(timer: Timer) {
    ...
    func tgc_heightForLabel(textLabel: UILabel, width: CGFloat) -> CGFloat {
        let attributes = [NSAttributedStringKey.font: textLabel.font!]
